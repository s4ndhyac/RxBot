...
var client = new TigerConnect.Client({
  baseUrl: '',
  defaultOrganizationId: '', // use the default org to send all messages in a specific organization unless specified otherwise
  events: {
    autoAck: false,
    isTyping: true,
    multiOrg: true,
...
client.signIn('', '', { udid: 'unique-device-id' }).then(function (session) {
  onSignedIn(session)
})
